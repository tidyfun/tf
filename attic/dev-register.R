...
w <- (tf_rgp(n, arg = t) / 2 + rnorm(n)) |>
  exp() |>
  tf_integrate(definite = FALSE)
w <- w / tf_fmax(w) * (max_t - min_t)
plot(w)

(warped <- tf_warp(x = rep(f0, n), warp = w)) |> plot()
tf_warp(x = rep(f0, n), warp = w) |> plot()
...
tf_unwarp(x = warped, warp = w_est) |> plot(col = 1:n)
lines(f0, col = 2)
...
layout(1)
tf_unwarp(x = warped, warp = w_est2) |> plot(col = 1:n)
...
tf_unwarp(x = warped, warp = w_est3) |> plot(col = 1:n)
# WORKS (badly!)
...
tf_unwarp(x = warped, warp = w_est4) |> plot(col = 1:n)
