...
# plot(reg_shift) would do basically the same here:
layout(t(1:3))
plot(
  pinch_small,
...
reg_fda_unpen <- tf_register(pinch_small, method = "fda")
inv_warp_fda_unpen <- tf_inv_warps(reg_fda_unpen)
reg_fda_pen <- tf_register(pinch_small, method = "fda", lambda = 1e-4)
inv_warp_fda_pen <- tf_inv_warps(reg_fda_pen)
```

SRVF and FDA both use non-linear warps and tend to be able to align more complex phase variation than the `affine` method, but their warps are more complex and potentially harder to interpret, and registration can go off the rails more easily (as we can see here with the unpenalized `fda`-registration plotted below, check the huge maximal warp slope and its poor amplitude variance reduction...) --
using `summary(<tf_registration>)` for quick quantitative diagnostics:
```{r pinch-compare-srvf-fda-summary, eval = have_srvf && have_fda, class.source = "fold-show"}
summary(reg_fda_unpen)
...
plot(pinch_small, main = "Original",
     col = alpha_palette, lwd = 1.5)
plot(inv_warp_fda_unpen, main = "FDA Inverse Warps (unpen.)", ylab = "",
     points = FALSE, col = alpha_palette, lwd = 1.5)
abline(0, 1, lty = 3)
plot(tf_aligned(reg_fda_unpen), main = "FDA Registered (unpen.)",
     col = alpha_palette, lwd = 1.5, points = FALSE)
plot(tf_aligned(reg_fda_unpen), main = "FDA Registered (unpen.)",
     type = "lasagna", col = hcl.colors(12, rev = TRUE))

plot.new()
plot(inv_warp_fda_pen, main = expression("FDA Inverse Warps (" * lambda * " = 1e-4)"),
     ylab = "", points = FALSE, col = alpha_palette, lwd = 1.5)
abline(0, 1, lty = 3)
plot(tf_aligned(reg_fda_pen), main = expression("FDA Registered (" * lambda * " = 1e-4)"),
     col = alpha_palette, lwd = 1.5, points = FALSE)
plot(tf_aligned(reg_fda_pen), main = expression("FDA Registered (" * lambda * " = 1e-4)"),
     type = "lasagna", col = hcl.colors(12, rev = TRUE))
```
