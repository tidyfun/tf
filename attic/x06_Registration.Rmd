...
In our benchmarks, SRVF and CC tended to degrade more gracefully under noise than affine and landmark methods. Pre-smoothing inputs with `tf_smooth()` before registration can help all methods.
...
The `srvf` and `cc` methods require regular grids (`tfd_reg`); interpolate sparse or irregular data to a common regular grid first. The `affine` and `landmark` methods also accept irregular grids (`tfd_irreg`). Grid density can affect results — try at least two grid resolutions to check stability.
...
reg_cc_unpen <- tf_register(pinch_small, method = "cc", max_iter = 10, nbasis = 10)
