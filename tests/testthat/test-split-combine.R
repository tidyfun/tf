test_that("tf_split works as expected", {
  ...
  expect_identical(tfs, tf_split(x, splits = c(0, 0.3, 0.5, 1)))

  expect_error(tf_split(x, splits = c(0.3, 0.5, 2)), "<= 1")
  expect_error(tf_split(x, splits = c(-2, 0.3, 0.5)), ">= 0")
...
test_that("tf_combine works as expected", {
  ...
  expect_identical(x, do.call(tf_combine, tf_split(x, 0.3, "left")))
  expect_identical(x_i, do.call(tf_combine, tf_split(x_i, 0.3, "left")))
  ...
  expect_error(
    tf_combine(x, tf_jiggle(x), strict = TRUE),
