...
test_that("fwise summaries work for tfb_fpc", {
  ...
  x_clamp <- (x - tf_fmin(x)) / (tf_fmax(x) - tf_fmin(x))
  expect_equal(tf_fmin(x_clamp), c(0, 0, 0), tolerance = .05) #!! uh oh
  expect_equal(tf_fmax(x_clamp), c(1, 1, 1), tolerance = .05) #!! uh oh

  x_std <- (x - tf_fmean(x)) / tf_fsd(x)
  expect_equal(tf_fmean(x_std), c(0, 0, 0), ignore_attr = TRUE, tolerance = .05)
  expect_equal(tf_fsd(x_std), c(1, 1, 1), ignore_attr = TRUE, tolerance = .05)
