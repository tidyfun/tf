...
one `which.max` index selects the same observation across every
component, so the result is never a "chimera" stitched together from
different curves. Note the deliberate divergence from
...
[`functionwise`](https://tidyfun.github.io/tf/dev/reference/functionwise.md),
[`tfsummaries`](https://tidyfun.github.io/tf/dev/reference/tfsummaries.md)
